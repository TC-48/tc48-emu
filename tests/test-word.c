...
#define TEST_WORD(NAME, UNAME, BITS, VALUES, SUFFIX)                        \
Test(word, NAME##_all) {                                                    \
    ...
    for (int iter = 0; iter < 1000; ++iter) {                               \
        ...
        cr_assert(tc48_##NAME##_to_u##SUFFIX(v) ==                          \
                  to_unsigned_ref(trits, BITS));                            \
                                                                            \
        cr_assert(tc48_##NAME##_to_i##SUFFIX(v) ==                          \
                  to_signed_ref(trits, BITS, VALUES));                      \
                                                                            \
        if (BITS > 1) {                                                     \
...
TEST_WORD(doublet,    DOUBLET,    TC48_DOUBLET_BITS,    TC48_DOUBLET_VALUES,    8b)
TEST_WORD(triplet,    TRIPLET,    TC48_TRIPLET_BITS,    TC48_TRIPLET_VALUES,    8b)
TEST_WORD(quadruplet, QUADRUPLET, TC48_QUADRUPLET_BITS, TC48_QUADRUPLET_VALUES, 8b)
TEST_WORD(tryte,      TRYTE,      TC48_TRYTE_BITS,      TC48_TRYTE_VALUES,      16b)
TEST_WORD(quarter,    QUARTER,    TC48_QUARTER_BITS,    TC48_QUARTER_VALUES,    32b)
TEST_WORD(half,       HALF,       TC48_HALF_BITS,       TC48_HALF_VALUES,       64b)
TEST_WORD(word,       WORD,       TC48_WORD_BITS,       TC48_WORD_VALUES,       128b)
